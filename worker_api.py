...
def burn_subtitles(video_path: str, ass_path: str, output_path: str):
    """Quema el archivo ASS en el video con FFmpeg, preservando resolución original."""
    cmd = [
        "ffmpeg", "-y",
        "-i", video_path,
        "-vf", f"ass={ass_path}",
...
def subtitle_video():
    ...
    import random
    style_id = request.form.get("style_id", random.choice(["A", "B", "C"])).upper()
    if style_id not in ("A", "B", "C"):
        style_id = "A"
    ...
    try:
        ...
        # 6. Quemar subtítulos
        burn_subtitles(video_in, ass_file, video_out)
